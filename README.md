...
# アプリケーションを作成した背景
今年小学校にあがったばかりの息子が、初めての自由研究にとまどっていてなかなか取り掛かることができなかったため。  
自由研究のアイデアを、子ども自身が興味のあることを出発点として楽しく見つけ、前向きな気持ちで取りかかれることを目指しました。  
人は、自分でやりたいと思ったことには積極的に取り組む一方で、他人から強要されると反発しがちです。自由研究はその自由さゆえに、何をすればよいのか分からずに戸惑ってしまう子どもたちも少なくありません。  
そうしたことから、このアプリケーションは、子どもたちが自由研究のアイデアを自分の好きなものを１つのきっかけとして、見つけることを目標にしています。「知ることの面白さ」を知り、「もっと学んでみよう」という心を大切にしたいと思っています。自分の興味や関心を学習のきっかけにして、次の知的好奇心に繋げることで学習の効果が高まり、長期的な成長にもつながる期待を込めています。

## 工夫したポイント
- アプリの使い勝手を向上させるために、複雑な機能を削減し、ユーザーが直感的に操作できるシンプルなUI設計にしました。  
...
- **新着表示機能**: 24時間以内の投稿には「NEW」表示が付きます。
- **新着順表示機能**: 新しい投稿から順に表示されるように、新着順（降順）に表示されます。
...
- **インフラ**: AWS (EC2, RDS)
- **OS**: Windows 11
- **その他**: GitHub, openAI API, Visual Studio Code (VSCode), Vim, PowerShell 7, コマンドプロンプト, AWS CloudShell, AWS CLI (Amazon Web Services Command Line Interface), Ubuntu (WSL - Windows Subsystem for Linux)
...
# 工夫したポイント
## API連携の難しさ
アプリケーション開発中に、OpenAIのAPIを使用してアイデア生成を行う際に、いくつかの技術的な課題に直面しました。特に、API連携がうまくいかず、期待通りの結果を得るのに時間がかかりました。

### 課題
初めてAPI連携を行った際、リクエストとレスポンスの形式が適切に設定されておらず、APIから期待するデータを取得できない状況が発生しました。また、APIキーの管理やセキュリティに関するベストプラクティスについても学ぶ必要がありました。

### 解決策
これらの問題を解決するために、まずはOpenAIおよびGithubの公式ドキュメントを徹底的に調査し、APIのリクエスト形式やエラーハンドリングの方法を理解しました。また、適切な環境変数の設定やAPIキーの管理方法を採用し、安全かつ効率的なAPI連携を実現しました。

最終的に、試行錯誤を繰り返した結果、API連携がスムーズに動作するようになり、ユーザーが入力したキーワードをもとにAIが自由研究のアイデアを生成できるようになりました。この過程を通じて、API連携に関する深い知識と経験を得ることができました。

## このアプリケーションを実際に使用した結果
開発段階から我が子にも実際に使用させ、いくつもの自由研究のアイデアを出しては親子で楽しく会話をしながらアイデアをさらに広げていきました。  
その結果、今年の自由研究は「ビスケットは何回たたくと1億個になるか」をテーマに無事完成させました。子どもは最後まで楽しく取り組めたようでよかったです。自ら学びたいと思えることを見つけるのが最も重要だと考えているので、そのきっかけになるようにしたいです。  
